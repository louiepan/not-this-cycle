...
// Import AFTER the window stub is in place to avoid issues with module-time
// environment checks (this module is safe because it gates on `typeof window`
// at call time, but importing late is the conservative choice).
const {
  loadProfile,
  saveProfile,
  appendRun,
...
describe('playerProfile', () => {
  describe('loadProfile', () => {
    ...
    it('rejects unknown schema versions', () => {
      window.localStorage.setItem(
        'ntc:player:v1',
        JSON.stringify({ schemaVersion: 99, playerName: 'ghost' })
      );
      const profile = loadProfile();
      expect(profile.playerName).toBe('');
    });
  ...
  describe('saveProfile + loadProfile round-trip', () => {
    it('preserves all fields', () => {
      ...
      const profile = loadProfile();
      expect(profile.playerName).toBe('Louis');
      expect(profile.lastDifficulty).toBe('senior');
    });
  });

  describe('updateIdentity', () => {
    it('updates name without losing history', () => {
      appendRun({
        difficulty: 'junior',
        archetype: 'the_ghost',
      ...
      const updated = updateIdentity('Louis', 'senior');
      expect(updated.playerName).toBe('Louis');
      expect(updated.lastDifficulty).toBe('senior');
      expect(updated.runHistory).toHaveLength(1);
    });

    it('leaves lastDifficulty alone when omitted', () => {
      saveProfile({
        schemaVersion: 1,
        playerName: '',
        lastDifficulty: 'principal',
        runHistory: [],
      });
      const updated = updateIdentity('Louis');
      expect(updated.lastDifficulty).toBe('principal');
    });
  });
