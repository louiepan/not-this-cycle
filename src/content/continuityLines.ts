 ...
 * commentary that reference prior reviews. The satire sharpens when the
 * game remembers you across cycles ("you have been here before, and
 * nothing has changed" is a stronger joke than "here is your rating").
 *
...
export interface ContinuitySelectionInput {
  ...
  playerName: string;
  /** Company name from the active ScenarioWorld (varies per session). */
  companyName: string;
}

interface CandidateLine {
  /** Higher tier (lower number) = more specific, more satirically loaded. */
  tier: 1 | 2 | 3 | 4;
  text: string;
  /** Category so we never emit two lines with the same theme. */
...
/**
 * Build all applicable candidate lines, sort by specificity, dedupe by
 * category, return at most 2.
 */
export function selectContinuityLines(input: ContinuitySelectionInput): string[] {
  const { history, current, playerName, companyName } = input;
  ...
  if (totalReviews === 2) {
    candidates.push({
      tier: 4,
      category: 'cadence',
      text: `Welcome back, ${nameOrYou}. This is your second performance review at ${company}. We appreciate your commitment to the process.`,
    });
  } else if (totalReviews >= 5) {
    candidates.push({
      tier: 3,
      category: 'cadence',
      text: `This is your ${ordinal(totalReviews)} performance review at ${company}. Several team members have asked HR if you are okay.`,
    });
  } else if (totalReviews >= 3) {
    candidates.push({
      tier: 4,
      category: 'cadence',
      text: `This is your ${ordinal(totalReviews)} performance review at ${company}. The pattern has not gone unnoticed.`,
    });
  }
    ...
    if (previousDifficulty !== current.difficulty) {
      ...
      if (moved > 0) {
        ...
        candidates.push({
          tier: 3,
          category: 'difficulty',
          text: `You have stepped down to ${labelForDifficulty(current.difficulty)}. This is not unusual after the year you have had.`,
        });
      }
    }
  }

  // --- SELECTION: top 1-2, prefer higher tier (lower number), category-deduped. ---
